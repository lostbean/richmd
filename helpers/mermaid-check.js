...
//   echo '{"blocks":[{"id":"a","source":"graph TD\n A --> B"}]}' \
//     | node helpers/mermaid-check.js
//
// The Lua filter invokes this ONCE PER DOCUMENT with every mermaid block in
// that document, not once per block — which is what makes the
// once-per-process DOM setup below actually pay off (see getMermaidParse).
//
// Exit code 0: every block valid, stdout is {"results":[{"id":...,"valid":true},...]}
// Exit code 1: at least one block rejected on grammar; that block's verdict
//   carries the parser's own reason, and its neighbours still get verdicts.
...
// test/mermaid-check.test.js) — the Lua filter shells out to this file as a
// subprocess, it never requires it in-process.
//
// The batch read/pair/report machinery lives in helpers/batch-cli.js and is
// shared verbatim with vega-lite-check.js: the only thing that differs
// between the two helpers is which check function runs per block, so the
// wire protocol has exactly one implementation and the two can never drift.
const isMain =
  process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1];
if (isMain) {
  runBatchCli(checkMermaid, "mermaid-check").then((code) => process.exit(code));
}
